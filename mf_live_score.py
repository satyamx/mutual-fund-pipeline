...
from mf_features import LEVEL_FEATURES, FeatureEngine, assemble_panel
from mf_infer import DEFAULT_TARGET, CohortInferencer, load_default
...
def build_live_panel(
    ...
    if codes is None:
        codes = [c for c in manifest["amfi_code"] if c in nav_panel and _asof(nav_panel[c], t)[0] is not None]
    if engine is None:
...
def _augmented_manifest(
    manifest: pd.DataFrame, code: str, category: str, series: pd.Series, sector: Optional[str] = None
) -> pd.DataFrame:
    ...
    row.update(
        amfi_code=str(code),
        scheme_name=f"<candidate {code}>",
        amc=None,
        category=category,
        sector=sector,
        first_nav_date=nav.index.min(),
        last_nav_date=nav.index.max(),
        n_obs=int(len(nav)),
    )
    return pd.concat([manifest, pd.DataFrame([row])], ignore_index=True)


def _z_candidate_from_reference(panel: pd.DataFrame, reference_codes: set, code: str) -> pd.DataFrame:
    ...
    for col in LEVEL_FEATURES:
        ...
        panel.loc[cand_mask, f"z_{col}"] = (panel.loc[cand_mask, col] - ref.mean()) / std if ok else np.nan
    return panel
...
def score_live(
    ...
    *,
    ...
    if code not in manifest_idx.index or not has_nav:
        ...
        if universe_index:
            verdict = mf_universe.classify(universe_index.get(str(code)), mf_universe.trained_categories(manifest))
        ...
        if candidate_category is None:
            return _gap_result(
                "NOT_IN_UNIVERSE", target, note="Fund not in the trained universe manifest, or no cached NAV."
            )
    ...
    if candidate_category is not None:
        ...
        engine, panel, _ref = build_candidate_panel(
            manifest, nav_panel, code, candidate_category, t, sector=candidate_sector
        )
    else:
    ...
    imputed = [
        c
        for c in inferencer.features
        if not (isinstance(target_features.get(c), (int, float)) and np.isfinite(target_features.get(c)))
    ]
    ...
    if _no_history(target_features):
        return _gap_result(
            ...
            n_features=len(inferencer.features),
    ...
    scoreable = [c for c in cohort_panel.index if not _no_history(cohort_panel.loc[c].drop(meta_cols[1:]).to_dict())]
    if len(scoreable) < COHORT_MIN_SIZE:
        return _gap_result(
            "THIN_COHORT",
            target,
            cohort_key=str(cohort_key),
            cohort_n=len(scoreable),
            universe_n=len(panel),
            anchor=str(t.date()),
            staleness_days=staleness_days,
    ...
    probs = {c: inferencer.predict(row.drop(meta_cols[1:]).to_dict(), target) for c, row in ranked.iterrows()}
    ...
    note = (
        "weak validated signal (holdout AUC ~%.3f) — supporting datapoint, never a verdict" % auc
    ...
    return dict(
        ...
        features={
            c: (float(v) if isinstance(v, (int, float)) and np.isfinite(v) else None)
...
def _selftest() -> None:
    ...
    if not np.all(np.abs(delta) <= 1e-12):
        ...
        bad = [c for i, c in enumerate(lcmp.columns) if not np.all(np.abs(delta[:, i]) <= 1e-12)]
        raise AssertionError(f"FAIL: live panel diverges from features.parquet at {bad} (max |Δ|={worst:.3e})")
    ...
    print(
        f"[selftest] Test A (feature parity @ {T.date()}, n={len(codes_T)}, "
    ...
    print(f"[selftest] Test B (prediction parity, {len(inf.targets)} targets): max|Δp|={max_err:.2e} — PASS")
    ...
    same_c = (
        live_sorted.drop(columns=["amfi_code", "anchor"])
        .round(12)
        .equals(trunc_sorted.drop(columns=["amfi_code", "anchor"]).round(12))
    )
    ...
    thin_manifest = manifest[~manifest["amfi_code"].isin([c for c in real_peers if c != target_code][2:])].reset_index(
        drop=True
    )
    res = score_live(
        target_code, manifest=thin_manifest, nav_panel=nav_panel, inferencer=inf, today=T + pd.Timedelta(days=1)
    )
    ...
    res = score_live(
        target_code, manifest=manifest, nav_panel=truncated, inferencer=inf, today=T + pd.Timedelta(days=45)
    )
    ...
    res = score_live(target_code, manifest=manifest, nav_panel=short, inferencer=inf, today=T + pd.Timedelta(days=1))
    assert res["status"] == "INSUFFICIENT_HISTORY", f"FAIL: expected INSUFFICIENT_HISTORY, got {res['status']}"
    assert res["probability"] is None, "FAIL: a refused fund must carry no probability"
    assert res["cohort_n"] >= COHORT_MIN_SIZE, "FAIL: cohort was thin, so this asserted the wrong gate"
    ...
    ok = score_live(target_code, manifest=manifest, nav_panel=nav_panel, inferencer=inf, today=T + pd.Timedelta(days=1))
    assert ok["status"] == "OK", f"FAIL: gate has no teeth — full history also refused ({ok['status']})"
    ...
    res2 = score_live(target_code, manifest=manifest, nav_panel=short2, inferencer=inf, today=T + pd.Timedelta(days=1))
    ...
    assert res2["cohort_n"] == ok["cohort_n"] - 1, (
        f"FAIL: cohort_n must report what was actually ranked over ({res2['cohort_n']} vs {ok['cohort_n']}-1)"
    )
    print(
        f"[selftest] INSUFFICIENT_HISTORY gate ({len(res['imputed_features'])} features "
    ...
    res = score_live("NOT-A-REAL-CODE", manifest=manifest, nav_panel=nav_panel, inferencer=inf, today=T)
    ...
    if uni:
        ...
        for kind, c in off.items():
            if c is None:
                continue
            res = score_live(c, manifest=manifest, nav_panel=nav_panel, inferencer=inf, today=T, universe_index=uni)
        ...
        print(
            f"[selftest] OUT_OF_TRAINING_UNIVERSE gate on real AMFI rows "
    ...
    for c in codes_T:
        ...
        if row["category"] not in SECTOR_KEYED_CATEGORIES:
            peers = [p for p in manifest[manifest["category"] == row["category"]]["amfi_code"] if p in nav_panel]
    ...
    naive_engine = FeatureEngine(_augmented_manifest(reduced, victim, vic_cat, nav_panel[victim]), nav_panel)
    naive = assemble_panel(naive_engine, pd.DataFrame({"amfi_code": list(ref_codes) + [str(victim)], "anchor": T}))
    ...
    assert drift > 1e-6, (
        f"FAIL: expected the naive panel to contaminate peers, but max "
        f"drift was {drift:.2e} — the guard may no longer be needed"
    )
    print(
        f"[selftest] naive single-panel build WOULD contaminate peers "
    ...
    assert spread > 1e-3, (
        f"FAIL: all {len(checked)} probabilities within {spread:.2e} — the "
        "comparison against truth would pass vacuously"
    )
    print(
        f"[selftest] {len(checked)} outsiders scored via insertion "
    ...
    sect = next(
        (c for c in codes_T if manifest.set_index("amfi_code").loc[c]["category"] in SECTOR_KEYED_CATEGORIES), None
    )
    if sect is not None:
        ...
        ov = {
            str(sect): mf_overrides.Override(
                amfi_code=str(sect), sector=str(true_sector), source_url="https://amc.example/sid.pdf"
            )
        }
        res2 = score_live(
            sect,
        ...
        if res2["status"] == "OK":
            truth_s = score_live(sect, manifest=manifest, nav_panel=nav_panel, inferencer=inf, today=T)
        ...
        eng_s = build_candidate_panel(red2, nav_panel, sect, "Sectoral/Thematic", T, sector=str(true_sector))[0]
        ...
        print(
            f"[selftest] curated sector unlocks {sect} -> {res2['status']}; "
            f"small_sectors + existing cohorts unchanged: PASS"
        )

    # ---- End-to-end sanity: a real fund scores OK with a stable-shaped dict --
    res = score_live(target_code, manifest=manifest, nav_panel=nav_panel, inferencer=inf, today=T)
    ...
    print(
        f"[selftest] end-to-end score_live({target_code}): status=OK "
        f"p={res['probability']:.3f} percentile={res['cohort_percentile']:.2f} "
        f"cohort_n={res['cohort_n']} universe_n={res['universe_n']} — PASS"
    )

    print(
        "[selftest] PASS — live scoring reproduces the training panel exactly, "
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--selftest", action="store_true", help="feature/prediction parity vs stored artifacts + anti-lookahead + gates"
    )
